...
HRESULT	InitialiseGraphics()//03-01
{
	...
	D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, "assets/trapMat.bmp", NULL, NULL, &g_pTexture0, NULL);
	g_pModelTwo = new Model(g_pD3DDevice, g_pImmediateContext);
	g_pModelTwo->LoadObjModel("assets/trap.obj");
...
void RenderFrame(void)
{
	...
	g_pModelTwo->SetYPos(-5);
	g_pModelTwo->LookAt_XZ(g_pCamera->GetX(),g_pCamera->GetZ());
	g_pModelTwo->MoveForward(0.01f);
	g_pModelTwo->Draw(&view, &projection);
